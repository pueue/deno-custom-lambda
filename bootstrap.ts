...
async function nextInvocation() {
  ...
  if (res.headers['lambda-runtime-trace-id']) {
    deno.env()._X_AMZN_TRACE_ID = res.headers['lambda-runtime-trace-id']
  } else {
    delete deno.env()._X_AMZN_TRACE_ID
  }
  ...
  if (res.headers['lambda-runtime-client-context']) {
    (context as any).clientContext = JSON.parse(res.headers['lambda-runtime-client-context'])
  }

  if (res.headers['lambda-runtime-cognito-identity']) {
    (context as any).identity = JSON.parse(res.headers['lambda-runtime-cognito-identity'])
  }

  const event = await res.body.json();
...
function getHandler() {
  ...
  let app
  try {
    app = import(LAMBDA_TASK_ROOT + '/' + modulePath)
  } catch (e) {
